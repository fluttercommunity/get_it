...
class GetIt {
  static final _factories = new Map<Type, _ServiceFactory<dynamic>>();
  ...
  static T  get<T>(Type type) {
    _ServiceFactory<T> object = _factories[type];
    if (object == null) {
  ...
  static void registerSingleton<T>(T instance) {
    assert(!_factories.containsKey(T),"Type ${T.toString()} is already registered");
   _factories[T] = new _ServiceFactory<T>(_ServiceFactoryType.constant, instance: instance);
  }
